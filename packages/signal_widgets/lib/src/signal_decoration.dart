...
/// A variant of [DecoratedBox] that evaluates based on a [SignalComputer<Decoration>].
class SignalDecoration extends SingleChildRenderObjectWidget {
  /// Creates a decoration widget using the provided [SignalComputer] callback.
  const SignalDecoration(
    this.decorate, {
    super.key,
  ...
  final SignalComputer<Decoration> decorate;
...
class _DecorationElement extends SingleChildSignalElement<RenderClippedDecoration> {
  _DecorationElement(super.widget);

  @override
  void recompute() {
    renderer.decoration = (widget as SignalDecoration).decorate(this);
  }
}
